...
// Публічний API ключ GIPHY (beta key)
const GIPHY_API_KEY = 'dc6zaTOxFJmzC'; // Або свій ключ
...
function init() {
    ...
    if(DOM.input) {
        DOM.input.addEventListener('keypress', (e) => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } });
    ...
    if(DOM.backBtn) DOM.backBtn.addEventListener('click', () => { document.getElementById('content-wrapper').classList.remove('chat-view-active'); activeChatRecipientId = null; });
...
function handleSearch(e) {
    ...
    if (q.length < 2) { if (q.length === 0) socket.emit('users_list_request'); return; }
    DOM.userList.innerHTML = '<li class="status">Пошук...</li>';
    fetch('/search_users', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({query: q}) })
    .then(r => r.json()).then(data => renderUserList(data.users, 'search')).catch(err => DOM.userList.innerHTML = '<li class="status">Помилка</li>');
}
...
function renderUserList(users, type='chats') {
    ...
    users.forEach(u => {
        ...
        const avatar = u.avatar_url ? `<img src="${u.avatar_url}" class="user-avatar-img">` : `<div class="user-avatar-placeholder">${u.username[0].toUpperCase()}</div>`;
        const subText = type === 'chats' ? `<span class="last-message">${u.last_message_text || ''}</span>` : `<span class="last-seen">${u.username}</span>`;
        const count = unreadCounts[u.id] || 0;
        li.innerHTML = `<div class="avatar-wrapper"><div class="user-avatar-container">${avatar}</div><div class="status-dot"></div></div><div class="user-info"><span class="username">${u.display_name || u.username}</span>${subText}</div><span class="unread-badge" style="display:${count > 0 ? 'block' : 'none'}">${count}</span>`;
...
function setupSocketHandlers() {
    ...
    socket.on('new_message', data => {
        ...
        if(pid === activeChatRecipientId) {
            ...
            if(li) { li.querySelector('.unread-badge').innerText = unreadCounts[pid]; li.querySelector('.unread-badge').style.display = 'block'; li.querySelector('.last-message').innerText = data.media_type==='text'?data.text:`[${data.media_type}]`; }
        }
    });
    socket.on('history_loaded', data => { chatHistories[data.partner_id] = data.history; if(activeChatRecipientId === data.partner_id) renderMessages(data.history); });
    socket.on('user_status_change', data => {
        if(data.status === 'online') online_users.add(data.user_id); else online_users.delete(data.user_id);
        if(activeChatRecipientId === data.user_id && DOM.chatStatus) { DOM.chatStatus.innerText = (data.status === 'online') ? 'Онлайн' : formatLastSeen(data.last_seen); DOM.chatStatus.className = `chat-status-subtitle ${data.status === 'online' ? 'online' : ''}`; }
    ...
    socket.on('reaction_updated', data => {
        for(let uid in chatHistories) { let m = chatHistories[uid].find(x => x.id === data.message_id); if(m) { m.reactions = data.reactions; break; } }
        if(activeChatRecipientId) {
             const li = document.querySelector(`li[data-message-id="${data.message_id}"]`);
             if(li) {
                 const oldR = li.querySelector('.message-reactions'); if(oldR) oldR.remove();
                 if(Object.keys(data.reactions).length > 0) {
                     ...
                     for(let [emoji, users] of Object.entries(data.reactions)) {
                         ...
                         html += `<span class="reaction-item ${my?'my-reaction':''}" onclick="window.react(${data.message_id}, '${emoji}')"><span class="reaction-emoji">${emoji}</span><span class="reaction-count">${users.length}</span></span>`;
                     }
                     html += '</div>';
    ...
    socket.on('message_deleted', data => {
         const pid = (data.sender_id === window.currentUserId) ? data.recipient_id : data.sender_id;
         if(chatHistories[pid]) { const m = chatHistories[pid].find(x => x.id === data.id); if(m) m.is_deleted = true; }
         if(pid === activeChatRecipientId) { const li = document.querySelector(`li[data-message-id="${data.id}"]`); if(li) { li.classList.add('deleted'); li.innerHTML = `<span>🚫 Повідомлення видалено</span><span class="timestamp">${formatUTCToLocal(data.timestamp)}</span>`; } }
    });
    
    socket.on('my_gifs_loaded', data => {
        DOM.gifLibrary.innerHTML = '';
        if(data.gifs.length === 0) { DOM.gifLibrary.innerHTML = '<div class="gif-loading">Тут пусто...</div>'; return; }
        data.gifs.forEach(url => { const img = document.createElement('img'); img.src = url; img.className = 'gif-item'; DOM.gifLibrary.appendChild(img); });
    });
}

function renderMessages(history) {
    DOM.messages.innerHTML = '';
    if (!history || history.length === 0) { DOM.messages.innerHTML = '<li class="status">Тут поки що пусто.</li>'; return; }
...
function appendMessage(msg, scroll=true) {
    ...
    if (!msg.is_deleted) {
        if (msg.reply_to) content += `<div class="message-reply-container"><div class="message-reply-author">${msg.reply_to.sender_name}</div><div class="message-reply-text">${msg.reply_to.is_deleted?'Видалено':(msg.reply_to.text||'[Медіа]')}</div></div>`;
        if (msg.forwarded_from) content += `<div class="message-forwarded">📤 Переслано від ${msg.forwarded_from.sender_name}</div>`;
    ...
    if (msg.reactions && Object.keys(msg.reactions).length > 0) {
        ...
        for(let [emoji, users] of Object.entries(msg.reactions)) {
             const my = users.some(u => u.user_id === window.currentUserId);
             reactionsHTML += `<span class="reaction-item ${my?'my-reaction':''}" onclick="window.react(${msg.id}, '${emoji}')"><span class="reaction-emoji">${emoji}</span><span class="reaction-count">${users.length}</span></span>`;
        }
        reactionsHTML += '</div>';
    }
    
    const menu = !msg.is_deleted ? `<div class="message-context-menu"><button class="context-menu-btn" onclick="window.reply(${msg.id})">↩️</button><button class="context-menu-btn" onclick="window.toggleReactions(${msg.id})">😀</button>${msg.sender_id === window.currentUserId ? `<button class="context-menu-btn delete-btn" onclick="window.del(${msg.id})">🗑️</button>` : ''}</div><div class="emoji-picker" id="emoji-${msg.id}"><div class="emoji-picker-grid">${['❤️','👍','😂','😮','😢','😡','🔥','🎉'].map(e => `<span class="emoji-picker-item" onclick="window.react(${msg.id}, '${e}')">${e}</span>`).join('')}</div></div>` : '';
    li.innerHTML = `${content} ${reactionsHTML} <span class="timestamp">${formatUTCToLocal(msg.timestamp)} ${(msg.sender_id===window.currentUserId)?(msg.is_read?'✓✓':'✓'):''}</span> ${menu}`;
...
function sendMessage() {
    ...
    socket.emit('send_message', {recipient_id: activeChatRecipientId, text: txt, media_type: isGifUrl(txt)?'gif':'text', media_url: isGifUrl(txt)?txt:null, reply_to_id: replyToMessage?replyToMessage.id:null});
    DOM.input.value = ''; hideReplyIndicator();
}

function handleTyping() { if(!activeChatRecipientId) return; if(!isTyping) { isTyping=true; socket.emit('typing_start', {partner_id: activeChatRecipientId}); } clearTimeout(typingTimeout); typingTimeout = setTimeout(() => { isTyping=false; socket.emit('typing_stop', {partner_id: activeChatRecipientId}); }, 2000); }
function handleFileSelect(e) { 
    const file = e.target.files[0]; if(!file || !activeChatRecipientId) return;
    const formData = new FormData(); formData.append('file', file); formData.append('recipient_id', activeChatRecipientId);
    fetch('/upload', {method: 'POST', body: formData}).then(r=>r.json()).catch(console.error);
    e.target.value = null; 
}

function createChatHeaderAvatar(user) {
    const old = document.getElementById('chat-header-avatar'); if(old) old.remove(); if(!user) return;
    ...
    // === ВИПРАВЛЕНО: Відкриття в тій самій вкладці ===
...
window.reply = (id) => { const msg = findMsg(id); if(msg) { replyToMessage = msg; showReplyIndicator(msg); DOM.input.focus(); } };
...
window.react = (id, e) => {
    ...
    const el = document.getElementById(`emoji-${id}`);
    if(el) el.style.display = 'none'; // Примусово закриваємо
};
...
function loadTrendingGifs() {
    ...
    .catch(e => DOM.gifLibrary.innerHTML = '<div class="gif-loading">Помилка (API Key?)</div>');
}
...
function searchGifs() {
    ...
    .then(r => r.json()).then(d => renderGifs(d.data))
    .catch(e => DOM.gifLibrary.innerHTML = '<div class="gif-loading">Помилка</div>');
}
